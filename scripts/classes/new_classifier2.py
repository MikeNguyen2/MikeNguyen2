...
def simple_pattern(points, rows, columns, margin1, margin2):
    ...
    if length_a > length_b:
        for column in range(columns):
            circles.append([])
            start_x = top_left[0] + (width1 - 2*margin_x)/(columns-1)*column - margin_x
            start_y = top_left[1] + (height1- 2*margin_y)/(columns-1)*column + margin_y
            
            for row in range(rows):
                pos_x = start_x + (width2 - 2*margin_x2)/(rows-1)*row + margin_x2
                pos_y = start_y + (height2- 2*margin_y2)/(rows-1)*row + margin_y2
