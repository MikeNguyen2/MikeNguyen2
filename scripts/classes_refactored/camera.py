...
class Camera:
    ...
    def set_color_options(self, exposure =166, gain = 64, brightness =0, contrast = 50, gamma = 300, hue = 0, saturation = 64, sharpness = 50, white_balance =4600):
        self.sensor.set_option(rs.option.exposure, exposure)
        self.sensor.set_option(rs.option.gain, gain)
        self.sensor.set_option(rs.option.brightness, brightness)
        self.sensor.set_option(rs.option.contrast, contrast)
        self.sensor.set_option(rs.option.gamma, gamma)
        self.sensor.set_option(rs.option.hue, hue)
        self.sensor.set_option(rs.option.saturation, saturation)
        self.sensor.set_option(rs.option.sharpness, sharpness)
        self.sensor.set_option(rs.option.white_balance, white_balance)
        self.sensor.set_option(rs.option.enable_auto_white_balance, True)

    # def set_depth_options(self):
    #     self.sensor_dep.set_option(rs.option.exposure, exposure)
    #     self.sensor_dep.set_option(rs.option.gain, gain)
    #     self.sensor_dep.set_option(rs.option.laser_power, laser_power)
    #     self.sensor_dep.set_option(rs.option.peak_threshold,peak_threshold)
